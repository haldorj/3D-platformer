...
{
	// Ensure game resolution does not exceed window size
...
void Application::InitGame(int gameResolutionWidth, int gameResolutionHeight)
{
    ...
    _GameState.MainCamera.Pitch = {0.0f};
    ...
    _GameState.LoadedFontGlyphs = LoadFontGlyphs("C:/Windows/Fonts/calibri.ttf", _Renderer.get());
	Assert(!_GameState.LoadedFontGlyphs.empty());
...
Texture Application::LoadTextureFromFile(const std::string& path)
{
...
void Application::UploadMeshesToGPU()
{
    Assert(_Renderer);
...
void Application::Move(float dt)
{
    ...
    auto& forward = _GameState.MainCamera.Direction;
...
void Application::UpdateGame(const float dt)
{
    ...
    for (int i = 0; i < MAX_ENTITIES; i++)
    {
		...
		M4 translation = MatrixTranslation(i * 2.5f, 0.0f, i * 2.5f);
		M4 rotation = MatrixRotationY(_GameState.Rot);
		M4 scale = MatrixScaling(1.0f, 1.0f, 1.0f);

		entity.WorldMatrix = scale * rotation * translation;
    }
}

std::unordered_map<char, FontGlyph> Application::LoadFontGlyphs(const std::string& path, Renderer* renderer)
{
